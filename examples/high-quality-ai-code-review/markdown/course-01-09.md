...
本章以教学化案例、构造的 44 条定向拒绝样本和已采纳对照样本为主要来源。样本用于诊断机制教学，不代表生产总体分布，也不用于计算总体采纳率。
...
本章以教学化案例、构造的采纳与拒绝样本，以及规则、上下文、复核和反馈回流方案为主要来源。代码片段均为教学化节选；开关与默认值以当前代码兜底配置为准，实际运行值可能由外部配置覆盖。
...
本章以课程案例的实际实验数据、当前代码实现和脱敏审查记录为主要证据来源。"AI 偷懒"标签仅用于引入现象，机制解释以可观察的衰减行为为准。实验数据来自教学化案例记录；不构成对任意模型或任务规模的通用断言。
...
本章以教学化的 AI Code Review 远端服务案例为主要来源。Git Worktree 的隔离语义参考 [Git 官方文档](https://git-scm.com/docs/git-worktree)。监控体系和并发策略为教学化案例设计。
...
本课程使用教学化的 AI Code Review 工程案例。代码片段和指标只用于说明方法，不构成对任意团队、模型或技术栈的通用预估。
